...
path_to_CDI_WG_und="/Users/elinlarsen/GoogleDrive/PhD_elin/Projets/CDSwordseg_Pipeline/CDI/english/instrument_data.csv"
...
path_to_CDI_WG_all="/Users/elinlarsen/GoogleDrive/PhD_elin/Projets/CDSwordseg_Pipeline/CDI/english/full_cdi_WG.csv"
...
d_WG_comprehension<-d_WG%>%
  filter(type == "word", value %in% c(0, "understands"))%>%
  rename(task_score=value, Type=definition)%>%
  select(data_id, task_score, age, Type, form)
...
df_WG_comp=get_parental_reports_nb_per_age(d_WG_comprehension, ages=c(8:30), "WG_comprehension")
...
png( paste(path_to_figures, "Distribution_parental_reports_all_forms_4.png", sep="") ,width=1800,height=1800, res=200 )
ggplot(data=df_nb_reports, aes(x=age, y=nb_reports, fill = form)) + 
