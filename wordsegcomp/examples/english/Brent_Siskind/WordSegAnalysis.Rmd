...
path_to_CDI="/Users/elinlarsen/Google Drive/CDSwordseg_Pipeline/CDI_wordbank/"
```
## Analysis of CDI database



***get CDI and reshape data***
...
data_per_infant<- read_csv(paste(path_to_CDI, "/instrument_data.csv", sep=""))
...
d_comprehension$comprehension=as.integer(d_comprehension$comprehension)
...
###3. rename definition by "Type" (i;e word type)
names(d_comprehension)[names(d_comprehension)=="definition"] <- "Type"
...
get_parental_reports_nb_per_age=function(data_CDI, ages=c(8:18))
{
df=setNames(data.frame(matrix(ncol = 2, nrow = 11)), c( "age", "parental_reports"))
i=0
for (a in ages)
{
  i=i+1
  gp<- data_CDI%>%
    ...
    group_by(data_id)
  df$age[i]=a
  df$parental_reports[i]=n_groups(gp)
}
return(df)
}
df=get_parental_reports_nb_per_age=function(data_CDI, ages=c(8:18))
print(df)
```
...
*** Add a prop column : proportion of infant reported to understand a word  ***
```{r}
add_prop_column=function(data_sampled, age_sampled_at)
{
  prop<-data_sampled%>%
      group_by(Type)%>%
      do(prop(x=.$comprehension))%>%
      mutate(age=age_sampled_at)%>%
      ungroup()
  return(prop)
}
```
...
```{r}
#test
s=sampling_CDI_reports(d_comprehension, 17, age_to_sample_at=8)
p=add_prop_column(s, 8)
p
```
...
#t<-average_sampling_CDI_reports(d_comprehension, 70, c(18), 1)
#t<-by_word_type(d_comprehension, 66, c(8:18), TRUE, 2)
test=by_word_type(d_comprehension, 66, c(8:18), FALSE, 1)
head(test)

test%>%group_by(age)%>%tally()
```
...
#lexical classes
lexClass <- read_delim("/Users/elinlarsen/Google Drive/CDSwordSeg_Pipeline/results/res-brent-CDS/Analysis_algos_CDI/CDI_lexical_classes.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
...
wordLength_cdi <- read_delim("/Users/elinlarsen/Google Drive/CDSwordSeg_Pipeline/results/res-brent-CDS/Analysis_algos_CDI/Mono_poly_CDI.csv", "\t", escape_double = FALSE, trim_ws = TRUE)%>%
...
ggplot(lc, aes(x=lexical_classes))+
  geom_bar()
```
...
```{r,  echo=FALSE, message=FALSE}
data_algo=merge(lc, wordLength_cdi, by='Type')
data_algo=merge(open_close_class, data_algo, by='Type')
data_algo=merge(as.data.frame(freq_algos), data_algo, by='Type')
```
...
#dat=merge(dd, data_algo, by="Type")
#R2=Linear_model(dat,c(8:18), c('TPs', 'PUDDLE', 'DiBs', 'AGu', 'gold'),c("syllable", "phoneme"))

png("/Users/elinlarsen/Google Drive/PhD_elin/our_papers/CogSci2018/figures/R2_over_ages_66sampled_10.png",width=4500,height=2500, res=200 )
...
ggplot(data=R2, aes(x = age, y = R2, colour=algo, group=unit)) +
  geom_point()+
  geom_errorbar(aes(ymin=R2-sd, ymax=R2+sd), colour="black", width=.1) +
